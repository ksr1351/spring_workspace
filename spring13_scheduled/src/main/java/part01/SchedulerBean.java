...
public class SchedulerBean { //따로 컨트롤러 선언 x , bean만 선언(scheduled-context)
	
	//SchedulerBean 클래스를 수행할 때 로그인 처리하기
	...
	System.out.println();을 사용하여 로그를 확인할 수 있으나 권장 X,  이보다는 로그를 기록하는 방법을 사용한다.
	...
	★INFO : 작업이 정상적으로 작동하고 있음을 메세지를 통해 확인하고 싶은 경우에 사용한다. (log4j.xml에서 확인 가능)
	...
	@Scheduled(cron="0/3 * * * * *") //시간에 상관없이 3초마다 아래 함수를 실행
	public void scheduleRun() {
		Calendar calendar = Calendar.getInstance();
		SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
		logger.info("스케쥴링 실행: " + sdf.format(calendar.getTime()));
...
● L : 일에서 사용하면 마지막 일, 요일에서는 마지막 요일(토요일) /Last
